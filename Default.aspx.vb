...
Partial Class _Default
 ...
 <WebMethod>
 Public Shared Function GetDadosGraficoD() As List(Of DadosDetalhesD)
  Dim dt As New DataTable()
  'Dim Conexao As String = "Server=localhost;Port=5432;UserId=postgres;Password=mktec;Database=Tecnocall"
  Dim Conexao As String = "Server=192.168.0.62;Port=5432;UserId=mktec;Password=mktec;Database=Tecnocall"
  Dim con As NpgsqlConnection = New NpgsqlConnection(Conexao)
  con.Open()
  Dim cmd As String
  cmd = "SELECT dt_referencia, cd_colaborador, nm_colaborador, tot_alo, tot_cpc,"
  cmd += " tot_venda, tot_acionamento, alo_acionamento, cpc_alo, venda_cpc, cpc_acionamento, "
  cmd += " venda_acionamento"
  ...
  Dim listaDadosD As New List(Of DadosDetalhesD)()
  For Each dtrow As DataRow In dt.Rows
   Dim details As New DadosDetalhesD()
   details.dt_referencia = dtrow(0).ToString()
   details.cd_colaborador = dtrow(1).ToString()
   details.nm_colaborador = dtrow(2).ToString()
   details.tot_alo = Convert.ToInt32(dtrow(3))
   details.tot_cpc = Convert.ToInt32(dtrow(4))
   details.tot_venda = Convert.ToInt32(dtrow(5))
   details.tot_acionamento = Convert.ToInt32(dtrow(6))
   details.alo_acionamento = Convert.ToSingle(dtrow(7))
   details.cpc_alo = Convert.ToSingle(dtrow(8))
   details.venda_cpc = Convert.ToSingle(dtrow(9))
   details.cpc_acionamento = Convert.ToSingle(dtrow(10))
   details.venda_acionamento = Convert.ToSingle(dtrow(11))

   listaDadosD.Add(details)
  Next
  Return listaDadosD
 End Function


 Public Class DadosDetalhesD
  Public Property dt_referencia() As String
